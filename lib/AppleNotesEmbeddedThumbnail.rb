...
class AppleNotesEmbeddedThumbnail < AppleNotesEmbeddedObject
  ...
  def initialize(primary_key, uuid, uti, note, backup, height, width)
  ...
  def get_media_filepath
    "Accounts/#{@note.get_account_identifier}/Previews/#{@filename}"
  end
  ...
  def get_thumbnail_extension
    return "png" if (@type != "com.apple.notes.gallery")
    return "jpg"
  end
